...
class VecDemoFrame extends Frame implements ComponentListener, ActionListener, MouseMotionListener, MouseListener,
	...
	class AuxBar {
		...
		AuxBar(Label l, DecentScrollbar b) {
			label = l;
			bar = b;
		}
	};
	...
	VecDemoFrame(VecDemo a) {
		super(BUILD_CASE_EMV("2-D Electrostatic Fields Applet v1.4b", null, "2-D Vector Fields Applet v1.4b"));
		applet = a;
	}
	...
	static String BUILD_CASE_EMV(String e, String m, String v) {
		return BUILD_V ? v : e;
	}

	static VecFunction BUILD_CASE_EMV(VecFunction e, VecFunction m, VecFunction v) {
		return BUILD_V ? v : e;
	}
	...
	public void initFrame() {
		...
		int i;
		for (i = 0; i != 27; i++)
			particleColors[i] = new Color(((i % 3) + 1) * 85, ((i / 3) % 3 + 1) * 85, ((i / 9) % 3 + 1) * 85);
		...
		for (i = 0; i != functionList.size(); i++)
			functionChooser.add("Setup: " + ((VecFunction) functionList.elementAt(i)).getName());
	...
	void handleResize() {
		...
		if (useBufferedImage) {
			try {
				...
				backimage = (Image) cstr.newInstance(new Object[] { new Integer(d.width), new Integer(d.height),
	...
	int computeColor(GridElement ge, double c) {
		...
		double redness = (value < 0) ? (java.lang.Math.log(-value) + offset) / range : 0;
		double grnness = (value > 0) ? (java.lang.Math.log(value) + offset) / range : 0;
		if (redness > 1)
			redness = 1;
		if (grnness > 1)
			grnness = 1;
		...
		if (redness < 0)
			redness = 0;
		/*
		 * if (redness > 0) System.out.print(redness + "\n");
		 */
	...
	int shadowBufferTop[], shadowBufferBottom[], shadowBufferTop2[], shadowBufferBottom2[];
		...
		for (x = xstart; x != xend; x += xdir) {
			for (y = ystart; y != yend; y += ydir) {
				...
				for (; x != xend; x += xdir) {
					...
					boolean vis = false;
					if (cx >= 0 && cx < winSize.width && cy <= shadowBufferTop[cx] && cy >= 0)
						vis = true;
					ge.visible = vis;
		...
		case FL_GRID:
			for (x = 0; x != gridsize; x++)
				for (y = 0; y != gridsize; y += 10) {
					...
					if (grid[x][y].visible) {
						map3d(nx, ny, grid[x][y].height, xpoints, ypoints, 0);
						map3d(nx1, ny, grid[x + 1][y].height, xpoints, ypoints, 1);
						drawLine(xpoints[0], ypoints[0], xpoints[1], ypoints[1]);
					}
					if (grid[y][x].visible) {
						map3d(ny, nx, grid[y][x].height, xpoints, ypoints, 0);
						map3d(ny, nx1, grid[y][x + 1].height, xpoints, ypoints, 1);
						drawLine(xpoints[0], ypoints[0], xpoints[1], ypoints[1]);
					}
				}
			break;
	...
	void interpPoint(GridElement ep1, GridElement ep2, int x1, int y1, int x2, int y2, double pval, FloatPair pos) {
	...
	void tryEdge(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4) {
		int i;
		double emult = 5;
...
//    }
	...
	void map3dView(double x, double y, double z, int xpoints[], int ypoints[], int pt, Rectangle view) {
		if (isFlat) {
		...
		if (z > 1000)
			z = 1000;
		/*
		 * if (!(z > -2000 && z < 2000)) System.out.print(z + " in map3d\n");
		 */
		...
		scaley = scalex;
		int yoff = (int) (scaley * (viewHeight - curfunc.getLevelHeight()) / viewDistance);
	...
	double getHeight(double x, double y) {
		...
		double fracx = x - ix;
		double fracy = y - iy;
		return grid[ix][iy].height * (1 - fracx) * (1 - fracy) + grid[ix + 1][iy].height * fracx * (1 - fracy)
				+ grid[ix][iy + 1].height * (1 - fracx) * fracy + grid[ix + 1][iy + 1].height * fracx * fracy;
	}
	...
	public void updateVecDemo(Graphics realg) {
		...
		if (xpoints == null)
			return;
		checkFlatState();
		barFieldStrength = fieldStrength = java.lang.Math.exp((strengthBar.getValue() - 50) / 10.);
	...
	double numIntegrate(double pos[], int n1, double x1, double x2, boolean line) {
		int steps = 8;
		double lastres = 0;
		double res = 0;
		int n2 = (line) ? n1 : 1 - n1;
	...
	void lineIntegralStep(Graphics g, int x, int y, double pos[], int dx, int dy, boolean line) {
		double field[] = rk_k1;
		curfunc.getField(field, pos);
		double f = (line) ? field[0] * dx + field[1] * dy : field[0] * dy - field[1] * dx;
		f *= reverse;
	...
	void genLines() {
		...
		for (i = 0; i != ct; i++) {
			...
			GridElement ge = grid[(int) ((p1x + 1) * gridsize / 2)][(int) ((p1y + 1) * gridsize / 2)];
			if (!ge.visible) {
				p.lifetime = -1;
				continue;
			}
			...
			ge = grid[(int) ((p.pos[0] + 1) * gridsize / 2)][(int) ((p.pos[1] + 1) * gridsize / 2)];
			if (!ge.visible) {
				p.lifetime = -1;
				continue;
			}
			...
			map3d(p.pos[0], p.pos[1], getHeight(p.pos[0], p.pos[1]), xpoints, ypoints, 1);
			drawLine(xpoints[0], ypoints[0], xpoints[1], ypoints[1]);
	...
	void drawVector(DrawData dd, double vec[]) {
		...
		map3d(vec[0] + sw2 * field[0], vec[1] + sw2 * field[1], 0, xpoints, ypoints, 1);
	...
	void moveParticles() {
		...
		for (i = 0; i != pcount; i++) {
			...
			moveParticle(pt);
			double x[] = pt.pos;
			if (!(x[0] >= -1 && x[0] < 1 && x[1] >= -1 && x[1] < 1) || (pt.lifetime -= timeStep) < 0)
				positionParticle(pt);
	...
	void drawParticles(Graphics g) {
		...
		for (i = 0; i < pcount; i++) {
			Particle p = particles[i];
			double pos[] = p.pos;
			GridElement ge = grid[(int) ((pos[0] + 1) * gridsize / 2)][(int) ((pos[1] + 1) * gridsize / 2)];
			map3d(pos[0], pos[1], getHeight(pos[0], pos[1]), xpoints, ypoints, 0);
			if (xpoints[0] < 0 || xpoints[0] >= winSize.width || ypoints[0] < 0 || ypoints[0] >= winSize.height)
				continue;
			if (disp == DISP_CURLERS) {
				g.setColor(p.color);
				final double len = .02;
	...
	void drawArrow(Graphics g, String text, int x1, int y1, int x2, int y2, int as) {
		...
		if (l > as / 2) { // was as
			double hatx = (x2 - x1) / l;
			double haty = (y2 - y1) / l;
			g.drawLine(x2, y2, (int) (haty * as - hatx * as + x2), (int) (-hatx * as - haty * as + y2));
			g.drawLine(x2, y2, (int) (-haty * as - hatx * as + x2), (int) (hatx * as - haty * as + y2));
			if (text != null)
	...
	static void rotateParticleAdd(double result[], double y[], double mult, double cx, double cy) {
	...
	void editView(int x, int y) {
		...
		if (modeChooser.getSelectedIndex() == MODE_LINE_INT || modeChooser.getSelectedIndex() == MODE_SURF_INT) {
			integralX = x;
			integralY = y;
			cv.repaint(pause);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == infoButton) {
			...
			try {
				...
				applet.getAppletContext().showDocument(new URL(applet.getCodeBase(), "functions.html" + '#' + s),
						"functionHelp");
			} catch (Exception ex) {
			}
		}
	...
	void dispChooserChanged() {
		...
		vecDensityBar.hide();
		/*
		 * lineDensityLabel.hide(); lineDensityBar.hide();
		 */
		partCountLabel.hide();
		partCountBar.hide();
		strengthLabel.show();
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getItemSelectable() == reverseCheck)
			functionChanged = true;
		if (e.getItemSelectable() == floorColorChooser || e.getItemSelectable() == floorLineChooser)
			backgroundChanged = true;
	}
	...
	void checkFlatState() {
		...
		int disp = dispChooser.getSelectedIndex();
		isFlat = flatCheck.getState() || curfunc.nonGradient() || disp == DISP_VECTORS || disp == DISP_CURLERS;
		int mode = modeChooser.getSelectedIndex();
	...
	void lineSegment(Particle p, int dir) {
		...
		for (i = 0; i != 3; i++)
			p.pos[i] = Y[i];
		p.phi = java.lang.Math.sqrt(ls_fieldavg[0] * ls_fieldavg[0] + ls_fieldavg[1] * ls_fieldavg[1]) / avgct;
	}
	...
	abstract class VecFunction {
		...
		void setGrid(GridElement ge, int x, int y) {
			...
			ge.valid = !boundCheck;
			/*
			 * if (!(ge.height >= -9999 && ge.height <= 9999)) System.out.print(xx[0] + " "
			 * + xx[1] + " " + ge.height + "\n");
			 */
			double xorig0 = xx[0];
			xx[0] += 1e-8;
	...
	class InverseRadial extends VecFunction {
		...
		String getName() {
			return BUILD_CASE_EMV("charged line", null, "1/r single line");
		}
	...
	class InverseRadialDouble extends VecFunction {
		...
		String getName() {
			return BUILD_CASE_EMV("line charge double", null, "1/r double lines");
		}
		...
		void getField(double result[], double y[]) {
			...
			double sep = gridToDouble(gridsize / 2 + aux1Bar.getValue() * gridsize / 200);
			double xx1 = y[0] - sep;
			double xx2 = y[0] + sep;
			...
			result[2] = .2 * (java.lang.Math.log(r1 + 1e-20) + sign * java.lang.Math.log(r2 + 1e-20));
		}
		...
		VecFunction createNext() {
			return BUILD_CASE_EMV(new InverseRadialDipole(), null, new InverseSquaredRadial());
		}
	};
...
//#ifdef BUILD_E
	...
	class InverseRadialQuad extends VecFunction {
		...
		void getField(double result[], double y[]) {
			...
			result[2] = .2 * (+java.lang.Math.log(r1 + 1e-20) - java.lang.Math.log(r2 + 1e-20)
					- java.lang.Math.log(r3 + 1e-20) + java.lang.Math.log(r4 + 1e-20));
		}
...
//#endif /* BUILD_E */
	...
	class InverseSquaredRadial extends VecFunction {
		String getName() {
			return BUILD_CASE_EMV("point charge", null, "1/r^2 single");
		}
	...
	class InverseSquaredRadialDouble extends InverseSquaredRadial {
		String getName() {
			return BUILD_CASE_EMV("point charge double", null, "1/r^2 double");
		}
		...
		VecFunction createNext() {
			return BUILD_CASE_EMV(new InverseSquaredRadialDipole(), null, new InverseRotational());
		}
	};
...
//#ifdef BUILD_E
	...
	class InverseSquaredRadialQuad extends InverseSquaredRadial {
		...
		void getField(double result[], double y[]) {
			...
			double r3 = distanceXY(xx1, yy2);
			double r4 = distanceXY(xx2, yy2);
			if (r1 < chargeSize || r2 < chargeSize || r3 < chargeSize || r4 < chargeSize)
				boundCheck = true;
			double q = .0003;
			double rq1 = q / (r1 * r1 * r1);
	...
	class ConductingPlate extends VecFunction {
		...
		void getField(double result[], double y[]) {
			// smythe p89
			if (y[1] >= -.02 && y[1] <= .02) {
				if ((plate && y[0] >= -a && y[0] <= a) || (!plate && (y[0] >= a || y[0] <= -a)))
					boundCheck = true;
			}
	...
	class ChargedPlate extends ConductingPlate {
		...
		double getPot(double a1, double a2, double y) {
			...
			if (y2 == 0)
				y2 = 1e-8;
			return .3 * (2 * (a1 - a2) + (b1 - cz.im) * y + a2 * java.lang.Math.log(a2 * a2 + y2)
	...
	class ChargedPlatePair extends ChargedPlate {
		...
		void getField(double result[], double y[]) {
			double sep = aux2Bar.getValue() / 100.;
			if ((y[1] >= -.01 + sep && y[1] <= .01 + sep || y[1] >= -.01 - sep && y[1] <= .01 - sep) && y[0] >= -a
					&& y[0] <= a)
				boundCheck = true;
			double a1 = -a - y[0];
			double a2 = a - y[0];
			...
			double q = .0003 / a;
			result[0] = .5 * q * (java.lang.Math.log((y12 + a2 * a2) / (y12 + a1 * a1))
					+ dipole * java.lang.Math.log((y22 + a2 * a2) / (y22 + a1 * a1)));
			result[1] = q * (java.lang.Math.atan(a1 / y1) - java.lang.Math.atan(a2 / y1)
					+ dipole * (java.lang.Math.atan(a1 / y2) - java.lang.Math.atan(a2 / y2)));
	...
	class Cylinder extends VecFunction {
		...
		void getField(double result[], double y[]) {
			...
			double cq = -pot / (mult * (java.lang.Math.log(a) - java.lang.Math.log(farpt)));
	...
	class CylinderAndLineCharge extends VecFunction {
		...
		void setupFrame() {
			...
			pot0 = -cq * java.lang.Math.log(a) + q * java.lang.Math.log(r2_0) - q * java.lang.Math.log(r3_0);
		...
		void calcDivergence() {
			...
			for (i = 0; i != 200; i++) {
				...
				grid[doubleToGrid(costh * a + getCylPos())][doubleToGrid(sinth * a)].div += (costh * res[0]
						+ sinth * res[1]) * 60 * reverse;
			}
		}
		...
		void getField(double result[], double y[]) {
			...
			result[2] = mult * (-pot0 - cq * java.lang.Math.log(r1) + q * java.lang.Math.log(r2 + 1e-20)
	...
	class CylinderInField extends VecFunction {
		...
		void getField(double result[], double y[]) {
			...
			if (r < a) {
				...
				if (conducting)
					...
					result[0] = (showD) ? e1 * fmult * (1 - dimult) : fmult * (1 - dimult);
		...
		void calcDivergence() {
			...
			for (i = 0; i != 200; i++) {
				...
				pos[2] = 0;
				curfunc.getField(res, pos);
				grid[doubleToGrid(costh * a)][doubleToGrid(sinth * a)].div += (costh * (rx - res[0])
						+ sinth * (ry - res[1])) * 4e2 * reverse;
			}
		}
	...
	/*
	 * class DielectricBoundaryE extends InverseSquaredRadial { boolean showD,
	 * conducting; double planeY; DielectricBoundaryE() { conducting = false; showD
	 * = false; } String getName() { return "dielec boundary"; } void setup() {
	 * setupBar(0, "Charge Location", 60); if (!conducting) setupBar(1,
	 * "Dielectric Strength", 60); } void getField(double result[], double y[]) {
	 * double cx = aux1Bar.getValue()/50.-1.001; double r1 = distance(y[0],
	 * y[1]-cx); if (r1 < chargeSize) boundCheck = true; double eps1 = 1; double
	 * eps2 = aux2Bar.getValue()/10. + 1; if (conducting) eps2 = 1e8; if (cx <
	 * planeY) { eps1 = eps2; eps2 = 1; } double q1 = .0003; double q2 =
	 * -(eps2-eps1)/(eps2+eps1)*q1; double ep = eps1; if (cx > planeY && y[1] <
	 * planeY || cx < planeY && y[1] > planeY) { q1 = 2*eps2*q1/(eps2+eps1); q2 = 0;
	 * ep = eps2; } double r2 = distance(y[0], y[1]-planeY*2+cx); result[2] =
	 * -2000*(q1/(r1*ep) + q2/(r2*ep)); if (!showD) { q1 /= ep; q2 /= ep; } double
	 * rq1 = q1/(r1*r1*r1); double rq2 = q2/(r2*r2*r2); result[0] = -y[0]*(rq1+rq2);
	 * result[1] = -(y[1]-cx)*rq1-(y[1]-planeY*2+cx)*rq2; } VecFunction createNext()
	 * { return new ConductingPlane(); } }; class ConductingPlane extends
	 * DielectricBoundaryE { ConductingPlane() { conducting = true; showD = false;
	 * planeY = -1; } String getName() { return "conducting plane + pt"; }
...
//#ifndef BUILD_E
	class InverseRotational extends InverseRadial {
		String getName() {
			return BUILD_CASE_EMV(null, "current line", "1/r rotational");
		}
	...
	class InverseRotationalDouble extends InverseRadialDouble {
		...
		String getName() {
			return BUILD_CASE_EMV(null, "current line double", "1/r rotational double");
		}
		...
		void getField(double result[], double y[]) {
			...
			if (showA) {
				if (dir2 == 1)
					result[2] += -.001 * (java.lang.Math.log(r) + java.lang.Math.log(r2) - 1);
				else
					result[2] += .001 * (java.lang.Math.log(r) - java.lang.Math.log(r2));
			} else {
				if (r < lineWidth * 2)
					boundCheck = true;
	...
	class InverseRotationalDoubleExt extends InverseRotationalDouble {
		...
		String getName() {
			return BUILD_CASE_EMV(null, "cur line double + ext", "1/r rot double + ext");
		}
	...
	class InverseRotationalDipole extends InverseRotationalDouble {
		...
		String getName() {
			return BUILD_CASE_EMV(null, "current line dipole", "1/r rotational dipole");
		}
	...
	class InverseRotationalDipoleExt extends InverseRotationalDouble {
		...
		String getName() {
			return BUILD_CASE_EMV(null, "cur line dipole + ext", "1/r rot dipole + ext");
		}
	...
	class OneDirectionFunction extends VecFunction {
		String getName() {
			return BUILD_CASE_EMV(null, "uniform field", "one direction");
		}
		...
		VecFunction createNext() {
			return BUILD_CASE_EMV(null, new MovingChargeField(), new InverseSquaredRadialSphere());
		}
	};
...
//#endif
	...
	/*
	 * #ifdef BUILD_E // this is a complex number class that avoids creating new
	 * objects // whenever possible. Takes a little more effort to use, but it is //
	 * much faster because we don't have to do constant garbage // collection. class
	 * Complex { public double a, b; Complex() { a = b = 0; } void set(double aa,
	 * double bb) { a = aa; b = bb; } void set(Complex c) { set(c.a, c.b); } void
	 * add(double r) { a += r; } void add(double r, double i) { a += r; b += i; }
	 * void square() { set(a*a-b*b, 2*a*b); } void mult(double c, double d) {
	 * set(a*c-b*d, a*d+b*c); } void mult(double c) { a *= c; b *= c; } void
	 * mult(Complex c) { mult(c.a, c.b); } void recip() { double n = a*a+b*b;
	 * set(a/n, -b/n); } void pow(double p) { double arg = java.lang.Math.atan2(b,
	 * a); arg *= p; double abs = java.lang.Math.pow(a*a+b*b, p*.5);
	 * set(abs*java.lang.Math.cos(arg), abs*java.lang.Math.sin(arg)); } void sin() {
	 * set(cosh(b)*java.lang.Math.sin(a), java.lang.Math.cos(a)*sinh(b)); } void
	 * cos() { set(cosh(b)*java.lang.Math.cos(a), java.lang.Math.sin(a)*sinh(b)); }
	 * void log() { set(java.lang.Math.log(a*a+b*b), java.lang.Math.atan2(b, a)); }
	 * void arcsin() { Complex z2 = new Complex(); z2.set(a, b); z2.square();
	 * z2.mult(-1); z2.add(1); z2.pow(.5); mult(0, 1); add(z2.a, z2.b); log();
	 * mult(0, -1); } };
	 * 
	 * double cosh(double a) { return
	 * .5*(java.lang.Math.exp(a)+java.lang.Math.exp(-a)); } double sinh(double a) {
	 * return .5*(java.lang.Math.exp(a)-java.lang.Math.exp(-a)); } #endif };
	 */
